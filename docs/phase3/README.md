...
- `tapi-orchestrator` sigue consumiendo el mismo contrato de mensaje SQS
...
npm run build
npm test -- --runInBand test/unit/producer/dynamodb-records.adapter.test.ts test/unit/producer/dispatch-records.test.ts test/unit/orchestrator/handler.test.ts test/unit/orchestrator/phase3-dispatch-compatibility.test.ts test/unit/cdk/tapi-stack.phase3.test.ts test/unit/cdk/tapi-stack.state-machine.test.ts
...
2. `Lambda > tapi-orchestrator` arranca una ejecucion de Step Functions
3. `Step Functions > tapi-consumer-state-machine` procesa sin cambios de contrato

Prueba recomendada:
- Invocar `payload-slot-149.json`
- Confirmar en logs de `tapi-orchestrator` que aparece `recordId: rec-201`
- Confirmar en `Execution input` de Step Functions que sigue existiendo `workItem + workflow`
...
- `tapi-orchestrator` sigue iniciando Step Functions sin cambios
